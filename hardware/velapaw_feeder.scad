...
part = "product"; // [product:full appliance + stand + camera aim (do NOT print), all:feeder mechanism only, rotor, housing, plate_stepper, plate_idle, hopper, board_cradle, board_cradle_stand, stand:assembled stand preview (do NOT print), base_front, base_back, support:print TWO, chute, pedestal, cam_pod:relocated OV5640 enclosure, cam_mast:drum-front camera yoke]
...
MOUNT_SCREW_D = 3.6;   // M3 through-holes, plates -> housing. Clearance, not a
                       ...
                       // around it is (2*POST_R - 3.6)/2 = 2.0mm.
POST_R       = 3.8;    // was 4.5. Shrunk with the MOUNT_R move below so the post
                       // outer edge lands at MOUNT_R + POST_R = 36.0 = PLATE_R:
                       // any bigger and the posts stand proud of the plate rims
                       // and become what the A-frame cup (SAD_R = PLATE_R + 1)
                       // touches first, lifting the plates off their journal.
MOUNT_R      = BORE_R + WALL + 0.2;   // 32.2 -- plate-screw bolt circle.
                       // Was BORE_R + WALL - 1 = 31, which put the 3.6 hole's
                       // inner edge at 29.2 -- only 0.2mm from the 29 bore. JLC's
                       // manual review (order G26080101901) red-flagged exactly
                       // those four full-height slivers as sub-1mm walls. At 32.2
                       // the ligament is 32.2 - 1.8 - 29 = 1.4mm. Costs 1.2mm of
                       // the plates' outboard margin (3.2 -> 2.0mm around the
                       // hole), fine in PA12. Used by the housing posts + holes,
                       // the plate_blank holes, and the hopper's post scallops --
                       // all four MUST stay on this one symbol.
...
module hopper() {
    difference() {
        ...
        // clears the drum's 32mm barrel, but the posts stand PROUD of it: r=POST_R
        // centred at radius MOUNT_R on the 45deg diagonals, bulging to 36.0.
        ...
        for (a = [45, 135])
            translate([cos(a) * MOUNT_R, 0,
                       -(BORE_R + WALL - HOP_DROP) + sin(a) * MOUNT_R])
                rotate([90, 0, 0])
...
BM_BASE_T   = 4;       // base plate thickness. 3 -> 4 in the anti-warp pass:
                       // the tray floor is a ~100x70 plate that can drum/bow in
                       // resin; walls and legs got thicker, the floor keeps up.
BM_WALL     = 4.0;     // nominal locating wall around the board. The pocket's
                       // BM_FIT clearance eats half of it, so what actually gets
                       // printed is BM_WALL - BM_FIT/2 = 3.0mm.
                       // Was 2.8 (printed 1.8, 1.3 at the bad corner) -- sized
                       // against JLCPCB's 1.2mm FDM minimum. Raised to 4.0 after
                       // JLC's checker flagged the stand as deformation-prone in
                       // resin: the tall pocket walls and the stand's legs (which
                       // take their width from BM_WALL) are the anti-warp
                       // stiffness, and 3.0mm printed resists the tray bowing.
                       // Costs 2.4mm on the outside envelope, nothing competes.
...
BM_CAM_WIN  = 24;      // camera window dia (centre). NOT sized by the lens barrel
                       ...
                       // The lens sits at z = BM_BASE_T + BM_STANDOFF = 16 and the
                       // binding aperture is the base's BOTTOM face at z = 0, so
                       // the throw is 16mm and the cone there is
                       //     dia = 2 * 16 * tan(FOV_diagonal / 2)
                       // = 20.4mm at 65deg, 23.2mm at 72deg. 14mm would have
                       // vignetted the corners off every frame -- and the pet
                       // detector would have been quietly cropped without any
                       // obvious symptom. 24 covers the usual 5MP OV5640 lens up
                       // to ~72deg diagonal at this throw (was 22 when the base
                       // was 3mm; grown in step with BM_BASE_T -- the anti-warp
                       // pass lengthened the window bore by 1mm).
...
BM_STRETCH_H = 35;     // rear stretcher height -- see board_cradle_stand().
                       // 35 puts a deeper shear web between the legs; its top-
                       // rear corner pokes ~3mm past the legs' sloped silhouette
                       // (cosmetic). Clear of the tray underside (rear-bottom
                       // edge at z~54) and of everything in the product pose.
...
module board_cradle_stand(show = false) {
    ...
    difference() {
        union() {
            ...
            bm_leg(-(OW/2 - BM_WALL/2), OH, lift);
            // REAR STRETCHER -- closes the two open legs into a portal frame.
            // JLC's checker (and physics) call the open two-leg wedge
            // deformation-prone in resin: each leg can rack independently and
            // the tray bows between them. The flat cut already grounds the
            // tray's front lip, so this one bar closes the footprint into a
            // ring: legs left/right, tray lip front, stretcher rear. It sits
            // on the HIGH (+Y) side, behind the camera's forward (-Y)
            // sightline; in the product it spans the pedestal's open centre
            // above the rails, clear of the wire drop (front-centre) and the
            // bowl rim (far below). Its top-rear corner pokes ~2mm past the
            // legs' sloped silhouette; cosmetic only.
            translate([-OW/2, OH/2 - BM_WALL, 0])
                cube([OW, BM_WALL, BM_STRETCH_H]);
        }
...
// ============================================================================
// CAMERA POD  --  the OV5640 RELOCATED off the board's back onto a 30cm FFC.
// ----------------------------------------------------------------------------
// The -C's camera is a bare OV5640 module that PLUGS into a 24-pin 0.5mm DVP
// connector (schematic J2, "0.5-24pin-cam"). Unplugging it and running a 30cm
// FFC + coupler lets the camera live anywhere -- so it no longer has to ride the
// board. The board stays on the pedestal as the SCREEN; this pod carries the
// lens out over the bowl and aims it at the pet.
//
// WHAT THIS PART IS: a small open-back enclosure that captures the bare module.
// It is deliberately built to the WIDE-ANGLE / MAX envelope, because the exact
// module was not calipered -- generic 24-pin OV5640 modules run 12.5..14.0mm
// square with a wide-angle barrel up to 14mm dia standing up to ~11mm proud.
// Sized to the top of that range, a smaller real part just sits looser and is
// taken up by adhesive; sized to the middle, a big one would not fit at all.
//
// RETENTION, and why there are no clamp tabs: with a 14mm barrel on a 14mm PCB
// the barrel eats almost the whole front face, so there is no front bezel to
// grab -- only the four PCB CORNERS (corner radius 9.9 > barrel radius 7.0) show.
// A rigid tab would also have to guess the PCB thickness, which varies. So the
// scheme is the one the module actually wants: the PCB drops into the pocket
// (located in X/Y by the walls), its front face lands on the FRONT-STOP LEDGE
// (the front plate, which the barrel pokes through), and a bead of silicone from
// the open back holds it. Nothing rigid touches the barrel -- which matters if it
// is an AUTO-FOCUS module, whose barrel is a moving VCM element and jams if
// pressed. The lens tip stands proud of the front plate, so the pod never
// vignettes even a 120deg cone.
POD_PCB     = 14.0;   // max square module PCB (wide-angle OV5640 envelope)
POD_FIT     = 0.6;    // per-axis pocket clearance (MJF; FDM takes it up in glue)
POD_WALL    = 2.5;    // pocket wall
POD_FRONT_T = 2.0;    // front plate = the front-stop ledge the PCB seats on
POD_DEPTH   = 6.0;    // pocket depth behind the ledge (PCB + rear components)
POD_BARREL  = 15.0;   // front aperture -- clears a 14mm barrel, lens pokes out
POD_FFC_W   = 13.0;   // FFC exit slot (24p @0.5 tail is 12.5 wide)
POD_FFC_H   = 2.5;    // FFC slot height
POD_EAR_L   = 8.0;    // side mounting ear reach (carries 2x M3 to the arm)
POD_EAR_T   = 3.0;    // ear thickness
POD_OUT     = POD_PCB + POD_FIT + 2*POD_WALL;   // 19.6 outer square
POD_H       = POD_FRONT_T + POD_DEPTH;          // 8.0 body height (front->back)
POD_POCK    = POD_PCB + POD_FIT;                // 14.6 pocket

// Local frame: the lens looks along -Z, the open back faces +Z, the FFC leaves
// through the +Y wall. The caller translates + tilts it to aim.
module cam_pod() {
    difference() {
        union() {
            // solid body
            translate([-POD_OUT/2, -POD_OUT/2, 0]) cube([POD_OUT, POD_OUT, POD_H]);
            // two side ears at the back, out of the -Z field of view. Inset 1mm
            // into the body so they FUSE (a coplanar touch reads as a 2nd solid).
            for (s = [-1, 1])
                translate([s > 0 ? POD_OUT/2 - 1 : -POD_OUT/2 - POD_EAR_L + 1,
                           -POD_OUT/2, POD_H - POD_EAR_T])
                    cube([POD_EAR_L + 1, POD_OUT, POD_EAR_T]);
        }
        // PCB pocket, open at the back (+Z)
        translate([-POD_POCK/2, -POD_POCK/2, POD_FRONT_T])
            cube([POD_POCK, POD_POCK, POD_DEPTH + 0.2]);
        // barrel aperture through the front plate
        translate([0, 0, -0.1]) cylinder(d = POD_BARREL, h = POD_FRONT_T + 0.2, $fn = 48);
        // FFC exit slot through the +Y wall
        translate([-POD_FFC_W/2, POD_POCK/2 - 0.1, POD_FRONT_T + 1])
            cube([POD_FFC_W, POD_WALL + 0.4, POD_FFC_H]);
        // M3 clearance through each ear
        for (s = [-1, 1])
            translate([s * (POD_OUT/2 + POD_EAR_L/2), 0, POD_H - POD_EAR_T - 0.1])
                cylinder(d = 3.4, h = POD_EAR_T + 0.2, $fn = 24);
    }
}

// DRUM-FRONT EYE. The camera sits centred just IN FRONT of the drum (a touch
// forward of the front A-frame and the red stepper plate) at drum-lower height,
// and looks STEEPLY DOWN-and-forward at the pet. The screen now faces the owner
// (front, sitting at Y -69..-150, z -20..61), so a level camera here would shoot
// into the back of the display -- but a steep-down ray from this low eye passes
// UNDER the screen's z=-20 bottom edge before it reaches it, and drops onto the
// pet over the bowl. It is carried by a slim two-leg yoke (cam_mast) that stands
// on base_front straddling the chute. Aim is COMPOUND in general (here X=0, so it
// reduces to a pure down-tilt): swing about Z by POD_YAW, tilt about X by
// POD_PITCH -> lays the pod's local -Z onto the (pod -> pet) vector.
POD_AT    = [0, -44, -13];           // centred, just in front of the drum, low
POD_DIR   = PET_ACT - POD_AT;        // pod -> pet
POD_LEN   = norm(POD_DIR);
POD_PITCH = acos(-POD_DIR[2] / POD_LEN);    // tilt about X
POD_YAW   = atan2(-POD_DIR[0], POD_DIR[1]); // swing about Z

// ============================================================================
// CAMERA YOKE (module name kept `cam_mast`)  --  the slim two-leg bracket that
// CARRIES the pod centred just in front of the drum. A SEPARATE printed part
// that bolts to base_front with two M3 (holes drilled in base_plate_solid at the
// foot XY). Two legs stand on the base STRADDLING the chute (feet at X = +/-34,
// chute is only X +/-23 wide) and lean inward as they rise, converging on a
// central pad that holds the pod's two ears. The centre stays open for the
// camera's own down-view; the legs are outboard of the sightline (X=0).
// ----------------------------------------------------------------------------
// Feet at Y=-44 land on base_front (front tile is Y < -32), in front of the
// drum (front face ~Y-28) and behind the bowl's back rim (Y-57), 85mm from the
// bowl centre so clear of the r71 locating ring.
YOKE_FX     = 34;               // foot X -- outboard of the chute (X +/-23)
YOKE_FY     = POD_AT[1];        // foot Y -- same plane as the pod (-44)
YOKE_BOTZ   = STAND_BASE_Z;     // rests on the base pocket floor top (-122)
YOKE_W      = 10;               // leg cross-section (X)
YOKE_D      = 12;               // leg cross-section (Y)
YOKE_KNEEZ  = -42;              // knee height: the legs rise VERTICALLY (at the
                                // foot X, outboard of the chute) up to here, then
                                // strut inward. The chute's rail crest in this Y
                                // band tops out at ~z-48, so a knee at -42 keeps
                                // the whole inward run 6mm+ clear above the trough.
MAST_PAD_T  = 6;                // pad plate thickness behind the pod ears
MAST_PILOT  = 2.6;              // M3 self-tap pilot in the pad

module cam_mast() {
    EAR_X = POD_OUT/2 + POD_EAR_L/2;
    color("DimGray") difference() {
        union() {
            // two legs. Each is a VERTICAL riser at X=+/-34 (outboard of the chute
            // the whole way down) from the base foot up to the knee, then a strut
            // that leans inward and up to the pad. Each strut lands ON ITS OWN EAR
            // (X = +/-EAR_X), NOT dead centre -- that keeps both attachments clear
            // of the central camera window, and the pad ring bridges the two into
            // one solid. Each cap runs the full pad thickness so it fuses solidly.
            for (s = [-1, 1]) {
                // vertical riser: foot -> knee, X held at s*YOKE_FX
                hull() {
                    translate([s * YOKE_FX - YOKE_W/2, YOKE_FY - YOKE_D/2, YOKE_BOTZ])
                        cube([YOKE_W, YOKE_D, 3]);
                    translate([s * YOKE_FX - YOKE_W/2, YOKE_FY - YOKE_D/2, YOKE_KNEEZ])
                        cube([YOKE_W, YOKE_D, 3]);
                }
                // inward strut: knee -> pad, landing at the ear (outside the window)
                hull() {
                    translate([s * YOKE_FX - YOKE_W/2, YOKE_FY - YOKE_D/2, YOKE_KNEEZ])
                        cube([YOKE_W, YOKE_D, 3]);
                    translate(POD_AT) rotate([0, 0, POD_YAW]) rotate([POD_PITCH, 0, 0])
                        translate([s * EAR_X - YOKE_W/2, -YOKE_D/2, POD_H - 3])
                            cube([YOKE_W, YOKE_D, MAST_PAD_T + 3]);
                }
            }
            // the pad the pod bolts onto, in the pod's tilted frame, with two
            // pilots aligned to the ears (holes at X = +/- (POD_OUT/2+POD_EAR_L/2))
            translate(POD_AT) rotate([0, 0, POD_YAW]) rotate([POD_PITCH, 0, 0])
                difference() {
                    translate([-(EAR_X + 4), -POD_OUT/2, POD_H - 0.2])
                        cube([2*(EAR_X + 4), POD_OUT, MAST_PAD_T + 0.2]);
                    // CAMERA WINDOW: cut the pocket-sized opening straight through
                    // the pad so the pod's OPEN BACK stays open -- that is how the
                    // OV5640 is seated and held (drop the PCB in, silicone bead
                    // from behind), and it gives the FFC tail a route up past the
                    // pad. A ~2.5mm lip all round still seats against the pod wall.
                    // The two ears (at X = +/-EAR_X = 13.8) sit well outside it.
                    translate([-POD_POCK/2, -POD_POCK/2, POD_H - 0.3])
                        cube([POD_POCK, POD_POCK, MAST_PAD_T + 1]);
                    for (s = [-1, 1])
                        translate([s * EAR_X, 0, POD_H - 0.3])
                            cylinder(d = MAST_PILOT, h = MAST_PAD_T + 1, $fn = 24);
                }
        }
        // one M3 clearance hole down through each foot into the base
        for (s = [-1, 1])
            translate([s * YOKE_FX, YOKE_FY, YOKE_BOTZ - 0.1])
                cylinder(d = 3.4, h = 14, $fn = 24);
    }
}
...
module base_plate_solid() {
    difference() {
        ...
        splice_bolt_voids();
        // 2x M3 through-holes for the camera yoke feet (back layout only; in
        // the front layout the camera rides the board, no yoke)
        if (LAYOUT != "front")
            for (s = [-1, 1])
                translate([s * YOKE_FX, YOKE_FY,
                           STAND_BASE_Z - STAND_BASE_T - 1])
                    cylinder(d = 3.4, h = STAND_BASE_T + 8, $fn = 24);
    }
}
...
module mount_back() {   // printable pedestal only (the wedge stand is a separate part)
    ...
    // the camera is no longer carried here -- it rides its own front mast
    // (cam_mast), a separate bolt-on part. The pedestal reverts to plain form.
}
...
module product() {          // whole appliance preview (do NOT print)
    ...
    // stand on the pedestal. The board is now UN-rotated (rotate 0) so the SCREEN
    // faces the owner (-Y, over the bowl). The board's own back camera is unused
    // in this layout -- the pet is watched by the decoupled front-mast pod below.
    if (LAYOUT == "front")
        screen_holder_placed();
    else
        translate([0, WEDGE_Y, PED_TOP]) rotate([0, 0, 0]) board_cradle_stand(true);
    color("BurlyWood") bowl();
    pet_head_marker();
    camera_sightline();
    // DRUM-FRONT camera pod on its two-leg yoke, centred just in front of the
    // drum and aimed steeply down-and-forward so the sightline (drawn below)
    // passes UNDER the owner-facing screen's bottom edge onto the pet.
    color([0.15,0.15,0.17])
        translate(POD_AT) rotate([0, 0, POD_YAW]) rotate([POD_PITCH, 0, 0]) cam_pod();
    cam_mast();
    color([0.15,0.7,1,0.9]) hull() {
        translate(POD_AT) sphere(1.2);
        translate(PET_ACT) sphere(1.2);
    }
}
...
else if (part == "cam_pod")      cam_pod();   // relocated OV5640 enclosure, front plate on the bed
else if (part == "cam_mast")     // drum-front yoke that carries the pod
    posed([0, 0, 0], [0, -YOKE_FY, -YOKE_BOTZ]) cam_mast();  // feet on the bed
