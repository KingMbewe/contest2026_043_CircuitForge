...
class AudioProcessor(object):
  ...
  def get_data(self, how_many, offset, model_settings, background_frequency,
    ...
    for i in range(offset, offset + sample_count):
      ...
      #
      # VELAPAW PATCH: upstream also ran self.merged_summaries_ here and pushed
      # the result into self.summary_writer_ -- a PNG-encoded spectrogram image
      # summary FOR EVERY CLIP, i.e. 100 encodes per step and 1.8M over an
      # 18,000-step run.  add_summary() queues to an unbounded in-memory queue
      # drained by a background thread, so when the producer outruns the disk
      # the queue grows without limit.  On this machine that wedged training
      # solidly: zero CPU, zero I/O, no step ever logged, 47 MB of tfevents
      # written and climbing, with only 2.6 GB free on C: for the pagefile.
      #
      # These images are debug output for the upstream tutorial that nobody
      # here will ever open.  The per-step scalar summaries in train.py (the
      # 'train' and 'validation' writers) are untouched and still work.
